...
let locationDropdownEl = document.getElementById("locationDropdown");
let typeDropdownEl = document.getElementById("typeDropdown");
let parkPlaceHolderEl = document.getElementById("parkPlaceHolder");

window.onload = () => {
  for (let i = 0; i < locationsArray.length; i++) {
    let locationOption = new Option(locationsArray[i]);
    locationDropdownEl.appendChild(locationOption);
  }

  for (let i = 0; i < parkTypes.length; i++) {
    let typeOption = new Option(parkTypes[i]);
    typeDropdownEl.appendChild(typeOption);

    // Get dropdown for types to show,
    // add eventHandler to both dropdowns
    // Console log selected value
    // Filter parks by chosen option
    // Reset and append the parks to display
  }

  locationDropdownEl.onchange = () => {
    console.log(locationDropdownEl.value);
    let locationVal = locationDropdownEl.value;
    parkPlaceHolderEl.innerHTML = "";
    // Filter out all parks that match the location
    // Append all parks to the page

    let newArray = nationalParks.filter((parks) => {
      return parks.State === locationVal;
    });
    newArray.forEach((park) => {
      let newDiv = document.createElement("div");
      newDiv.textContent = park.LocationName;

      parkPlaceHolderEl.appendChild(newDiv);
    });
  };
  
  typeDropdownEl.onchange = () => {
    console.log(typeDropdownEl.value);
    let typeVal = typeDropdownEl.value;
    parkPlaceHolderEl.innerHTML = "";
    // Filter out all parks that match the location
    // Append all parks to the page

    let newArray = nationalParks.filter((parks) => {
      return parks.LocationName.toLowerCase().includes(typeVal.toLowerCase());
    });
    newArray.forEach((park) => {
      let newDiv = document.createElement("div");
      newDiv.textContent = park.LocationName;
